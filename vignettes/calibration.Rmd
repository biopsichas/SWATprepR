...
Data, which are prepared according to given templates (*'calibration_data.xlsx'*), can be loaded directly with package function [load_template](../reference/load_template.html). Two data types are loaded. First is station information (location, name, ID). Second is values from those monitoring stations with IDs to relate to station, variable names, dates and values. Function needs only paths to the template and EPSG code (if different from 4326) for point coordinates. 
