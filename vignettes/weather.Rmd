...
Nitrogen atmospheric deposition data can be extracted from European Monitoring and Evaluation Programme (EMEP) provided [data files](https://www.emep.int/mscw/mscw_moddata.html). The EMEP portal provides gridded 0.1 degress resolution netCDF
data over Europe. The parameters of interest are: 
...
- *NH4_RF* - ammonia deposition (kg/ha/yr);
- *NO3_DRY* - nitrate dry deposition (kg/ha.yr). 

EMEP data provides access to yearly, monthly, daily and hourly data. Data for a selected catchment with required time step could be extracted directly from [EMEP website](https://www.emep.int/mscw/mscw_moddata.html)  with [get_atmo_dep](../reference/get_atmo_dep.html) function. More detailed (yearly to hourly) data selected, the longer function will be running. More information about the inclusion of atmospheric deposition data in SWAT+ modeling is presented in the section 3.11 of [SWAT+ modelling protocol](https://doi.org/10.5281/zenodo.7463395).
