...
[prepare_climate](../reference/prepare_climate.html) function can be used to prepare/update SWAT+ model input text files for weather data. The function prepares following files:
...
The function also updates *aquifer.con*, *chandeg.con*, *hru.con*, *reservoir.con*, *rout_unit.con* and *time.sim* SWAT+ model input files. Following line provide an example, how it could be applied. 

```{r update2, eval=FALSE, message = FALSE, warning = FALSE}
